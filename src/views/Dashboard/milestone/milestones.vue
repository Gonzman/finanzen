    ...
    for (const milestone of milestones.value) {
        try {
            ...
            milestoneTransactions.value[milestone.id] =
                transactions as TransactionResponse<ExpandedTransaction>[];
        } catch (error) {
            console.error(
                `Error fetching transactions for milestone ${milestone.id}:`,
                error,
            );
            milestoneTransactions.value[milestone.id] = [];
        }
    ...
    return milestoneTransactions.value[milestoneId].reduce(
        (
            ...
            console.log(
                ...
                transaction.expand?.transactionAuth_via_transaction?.[0]
                    ?.state ?? 'undefined',
            );
            if (
                transaction.expand?.transactionAuth_via_transaction?.[0]
                    ?.state === TransactionAuthStateOptions.Autorisiert
            ) {
...
<template>
    <div class="mb-4 flex justify-between items-center">
        <Input
            class="max-w-[400px]"
            placeholder="Suche nach Meilensteinen"
            v-model="filter"
        />
            ...
            >
                ...
                <TableCell>
                    <MilestoneTransactionStats
                        ...
                        :transactions="
                            milestoneTransactions[milestone.id] || []
                        "
                    />
                </TableCell>
                ...
                <TableCell
                    class="text-right"
                    :class="
                        getApprovedAmount(milestone.id).konto < 0
                            ? 'text-red-500'
                            : 'text-green-500'
                    "
                >
                ...
                <TableCell
                    class="text-right"
                    :class="
                        getApprovedAmount(milestone.id).bar < 0
                            ? 'text-red-500'
                            : 'text-green-500'
                    "
                >
                ...
                <TableCell
                    class="w-0 p-0"
                    :class="
                        getApprovedAmount(milestone.id).konto +
                            getApprovedAmount(milestone.id).bar <
                        0
                            ? 'text-red-500'
                            : 'text-green-500'
                    "
                ...
                <TableCell class="text-right w-0 p-0">
                    <DropdownMenu>
                        <DropdownMenuTrigger as-child>
                            <Button
                                variant="ghost"
                                class="w-9 h-9 p-0 data-[state=open]:bg-muted"
                            >
                                …
                            </Button>
                        </DropdownMenuTrigger>
