...
class pb {
    ...
    refreshMilestones(ausschuss?: string) {
        ...
        if (currentAusschuss) {
            ...
            return this.client
                ...
                .getFullList<MilestoneResponse<ExpandMilestones>>({
                    expand: 'transaction',
                    sort: '-updated',
                })
                .then(result => {
    ...
    //Old API ruleset: (@request.auth.verified = true && transaction.ausschuss.users.id ?= @request.auth.id) || @request.auth.isPruefer = true
